...
export default function ConverterPage() {
  ...
  const [activeTab, setActiveTab] = useState<'view' | 'code' | 'help'>('view');
  ...
  // Run AI analysis utilizing Gemini-3.5-flash
  const triggerAiDiagnose = async () => {
    ...
    try {
      const response = await fetch('/api/gemini/analyze', {
        ...
        body: JSON.stringify({
          ...
          shapeType: originalGeoJson.features?.[0]?.geometry?.type || '混合类型',
          bbox: mapBBoxRef.current,
          featureCount: origStats.features,
          vertexCount: origStats.vertices,
          attributes: propertySamples,
          fileSizeOriginal: origStats.size,
        }),
      });
      ...
      if (data.success) {
        ...
        setActiveTab('code'); // Switch to guidance viewport
      } else {
        setAiReport('诊断生成产生缺陷：' + data.error);
      }
    } catch (err: any) {
      console.error(err);
      setAiReport('网络请求遇到故障：' + err.message);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-slate-300 flex flex-col" id="app_root">
      ...
      <main className="flex-1 max-w-[1720px] w-full mx-auto p-6 flex flex-col lg:flex-row gap-6">
            ...
            <div className="bg-[#2DD4BF]/5 rounded-lg p-3.5 border border-[#2DD4BF]/15 flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-[#2DD4BF]/80 mt-1 leading-relaxed">
                  微信小程序地图在中华范围内强制使用火星坐标。纠偏后，能够完美贴合街道路网，拒绝飘移。
                </p>
              </div>
            </div>
        ...
        <div className="flex-1 min-w-0 space-y-6 flex flex-col" id="output_panel">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-stretch">
            {/* Visual View Box (Ref Canvas) */}
            <div className="xl:col-span-7 bg-[#0D0D0D] border border-[#1A1A1A] rounded-lg p-4 flex flex-col min-h-[460px] relative">
              ...
              {processedGeoJson ? (
                <div className="relative flex-1 bg-[#070707] rounded overflow-hidden border border-[#1C1C1C] shadow-inner group">
                  <canvas
                    ref={canvasRef}
              ...
              {/* Download CTA Float Bottom Card */}
              {processedGeoJson && (
                <div className="mt-4 bg-[#141414] border border-[#262626] p-4 rounded flex flex-col sm:flex-row items-center justify-between gap-3 animate-fadeIn">
                  <div className="flex items-center space-x-2">
                    <CheckCircle className="h-5 w-5 text-[#2DD4BF] flex-shrink-0" />
                    <div>
                      <p className="text-xs font-semibold text-white">GeoJSON 输出就绪</p>
                      <p className="text-[10px] text-slate-500 mt-0.5">火星坐标转换成功，体积已被有效裁切</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2 w-full sm:w-auto">
                    <button
                      onClick={handleDownloadGeoJSON}
                      className="w-full sm:w-auto bg-[#2DD4BF] hover:bg-[#2DD4BF]/90 text-black px-4 py-2.5 rounded text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer border-0 shadow-lg"
            ...
            {/* Diagnostic Sidebar with Gemini AI and WeChat boilerplate generator */}
            <div className="xl:col-span-5 flex flex-col bg-[#0D0D0D] border border-[#1A1A1A] rounded-lg p-4 min-h-[460px] relative">
              <div className="border-b border-[#1A1A1A] pb-3 mb-4 flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <Cpu className="h-4 w-4 text-[#2DD4BF] animate-pulse" />
                  <span className="text-xs font-bold text-white uppercase tracking-[0.1em]">AI Diagnostic GIS Optimization</span>
                </div>
                {processedGeoJson && (
                  <button
                    onClick={triggerAiDiagnose}
                    disabled={isParsing || isAiLoading}
                    className="bg-black border border-[#262626] hover:bg-[#141414] text-white text-[11px] font-semibold px-3 py-1.5 rounded shadow-sm font-sans flex items-center gap-1.5 transition cursor-pointer disabled:opacity-40 select-none hover:scale-[1.01]"
                    id="btn_ai_diagnose"
                  >
                    {isAiLoading ? (
                    ...
                    {isAiLoading ? 'AI 诊断计算中...' : 'AI 诊断本项图层'}
                  </button>
                )}
              </div>

              {/* Panel tabs */}
              <div className="flex border-b border-[#1A1A1A] mb-3 text-xs">
                <button
                  type="button"
                  onClick={() => setActiveTab('view')}
                  className={`px-3 py-2 border-b-2 font-medium transition cursor-pointer ${activeTab === 'view' ? 'border-[#2DD4BF] text-[#2DD4BF]' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-2 border-b-2 font-medium transition cursor-pointer ${activeTab === 'code' ? 'border-[#2DD4BF] text-[#2DD4BF]' : 'border-transparent text-slate-400 hover:text-slate-200'}`}
                >
                  <span className="flex items-center gap-1.5">
                    <Brackets className="h-3.5 w-3.5" />
                    AI 诊断和优化 ({aiReport ? '已生成' : '空'})
                  </span>
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto max-h-[460px] scrollbar-thin">
                ...
                {/* Tab 1: Attribute Inspector details */}
                {activeTab === 'view' && (
                  <div className="space-y-4 animate-fadeIn">
                    {selectedFeature ? (
                      <div className="space-y-3">
                        <div className="bg-[#141414] rounded p-3 border border-[#262626]">
                          <div className="flex items-center justify-between border-b border-[#262626] pb-2 mb-2">
                            ...
                            <button
                              ...
                              className="text-[10px] bg-black border border-[#262626] hover:bg-[#1C1C1C] px-2 py-0.5 rounded text-slate-300 cursor-pointer"
                            >
                              取消选中
                            </button>
                          </div>
                          <div className="divide-y divide-[#1A1A1A] font-mono text-xs">
                            {Object.keys(selectedFeature.properties || {}).map((k) => (
                              <div key={k} className="flex justify-between py-1.5 pr-1 font-mono">
                                <span className="text-slate-550 font-medium">{k}:</span>
                                <span className="text-slate-300 font-semibold truncate max-w-[220px]">{String(selectedFeature.properties[k])}</span>
                              </div>
                            ))}
                          </div>
                        </div>

                        <div className="bg-[#2DD4BF]/5 border border-[#2DD4BF]/15 rounded p-3 text-xs text-slate-300 space-y-1">
                          <p className="font-semibold text-[#2DD4BF]">💡 提示：</p>
                          <p className="leading-relaxed text-slate-400">
                            当前要素包含 {countVertices(selectedFeature)} 个折线/多边形节点。微信小程序中可以直接通过多边形渲染属性（对应 <code className="bg-black/40 text-[#2DD4BF] border border-[#262626] px-1 font-mono rounded">polygons</code> 数组元素）以此渲染出来并在页面中展示。
                          </p>
                        </div>
                      </div>
                    ) : (
                      <div className="py-12 text-center">
                        <Layers className="h-10 w-10 text-[#262626] mx-auto mb-3" />
                        <h4 className="text-xs font-bold text-slate-400">未选中任何要素</h4>
                        <p className="text-xs text-slate-500 mt-1 max-w-[240px] mx-auto">
                          交互预览区域中支持点击要素或地块。点击后在此面板会渲染出大块属性及要素坐标的详细物理描述。
                        </p>
                      </div>
                    )}
                  </div>
                ...
                {/* Tab 2: AI Report details */}
                {activeTab === 'code' && (
                  <div className="space-y-4 animate-fadeIn">
                    {isAiLoading ? (
                      <div className="py-20 text-center flex flex-col items-center justify-center space-y-3">
                        <RefreshCw className="h-8 w-8 text-[#2DD4BF] animate-spin" />
                        <span className="text-slate-400 text-xs">正在进行 AI 疏通诊断分析...</span>
                      </div>
                    ) : aiReport ? (
                      <div className="bg-[#141414] rounded p-4 border border-[#262626] font-mono leading-relaxed text-slate-300 text-xs whitespace-pre-wrap select-text max-h-[380px] overflow-y-auto">
                        {aiReport}
                      </div>
                    ) : (
                      <div className="py-12 text-center">
                        <Cpu className="h-10 w-10 text-[#262626] mx-auto mb-3" />
                        <h4 className="text-xs font-bold text-slate-400">尚无诊断数据</h4>
                        <p className="text-xs text-slate-500 mt-1 max-w-[240px] mx-auto">
                          点击右上角的“AI 诊断本项图层”按钮，由 AI 模型为您运算生成空间剖析优化建议。
                        </p>
                      </div>
                    )}
                  </div>
      ...
      <footer className="border-t border-[#1A1A1A] py-6 px-6 text-center text-[11px] text-slate-600 bg-black">
        <p>© 2026 Shapefile-to-WeChat-GeoJSON Map Compressor. Powered by Google AI Studio Gemini-3.5-flash & Proj4 Client Library.</p>
      </footer>
    </div>
  );
}
